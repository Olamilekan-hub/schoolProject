...
const FingerprintScanner: React.FC<BiometricScannerProps> = ({
  ...
  const { isSupported, isAvailable, startScan, stopScan, deviceInfo } =
    useBiometric();
  ...
  const handleStartScan = useCallback(async () => {
    if (!isSupported) {
      const errorMsg =
        "Windows Hello or biometric authentication is not supported on this device";
    ...
    if (!isAvailable) {
      const errorMsg =
        "Windows Hello is not set up or no fingerprints are enrolled. Please set up Windows Hello in Settings.";
      toast.error(errorMsg);
      const result: BiometricScanResult = { success: false, message: errorMsg };
      onScanResult(result);
      return;
    ...
    try {
      ...
      if (scanResult.success) {
        ...
        throw new Error(
          scanResult.message || "Fingerprint verification failed"
        );
      }
    } catch (error: any) {
      ...
      if (error.name === "NotAllowedError") {
        errorMsg =
          "Windows Hello access was denied. Please try again or check your Windows Hello settings.";
      } else if (error.name === "SecurityError") {
        errorMsg =
          "Security error. Please ensure you are using HTTPS and Windows Hello is properly configured.";
      ...
      >
        <div className="text-center">
          ...
          <p className="text-center text-warning-700 mb-4">
            Windows Hello or biometric authentication is not supported on this
            device or browser.
          </p>
          <div className="text-sm text-warning-600 space-y-">
            <p>
      ...
      <div className="text-center">
        ...
        {deviceInfo && (
          <div className="text-xs text-gray-500 mb-4 p-3 bg-gray-100 rounded-lg">
            ...
            <div className="space-y-1">
              ...
              <p>
                Status: {deviceInfo.isConnected ? "Connected" : "Disconnected"}
              </p>
            </div>
          </div>
        )}
        ...
        {scannerState === "idle" && isAvailable && (
          <div className="mb-4 p-3 bg-blue-50 rounded-lg text-sm text-blue-800">
            ...
            <p>
              When you click scan, Windows will prompt you to verify your
              fingerprint.
            </p>
          </div>
        )}
