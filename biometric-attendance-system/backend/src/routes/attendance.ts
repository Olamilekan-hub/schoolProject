...
import { validateAttendanceSession, validateMarkAttendance } from '../middleware/validation'
...
router.post('/sessions', authenticate, validateAttendanceSession, async (req, res) => {
  try {
    const {
      courseId,
      sessionName,
    ...
    const session = await prisma.attendanceSession.create({
      data: {
        ...
        allowRemoteMarking,
        attendanceLinkToken: allowRemoteMarking ? generateAttendanceLink() : null,
        linkExpiresAt: allowRemoteMarking ? new Date(Date.now() + 24 * 60 * 60 * 1000) : null
      },
      include: {
        course: true
      }
...
router.post('/mark', authenticate, validateMarkAttendance, async (req, res) => {
  try {
    ...
    logger.info(`Remote attendance marked: ${student.matricNumber} for session ${session.id}`)
